...
namespace UMiniFramework.Editor.UMEModules.Config
{
    public class UMConfigHandler
    {
        ...
        public static void UpdateConfig(List<string> configFiles, string scriptFolder, string dataFolder)
        {
            ...
            for (var i = 0; i < configFiles.Count; i++)
            {
                ...
                EditorUtility.DisplayProgressBar("UMConfigModule Create Config By Excel",
                    $"Current Excel: {currentExcel}", i / configFiles.Count);
                CreateConfigByExcel(currentExcel);
            }

            EditorUtility.ClearProgressBar();
            AssetDatabase.Refresh();
            EditorUtility.DisplayDialog("Tip", "Configuration update complete.", "OK");
        }
        ...
        private static void CreateConfigScript(List<ConfigFieldInfo> fieldInfos, string excelPath)
        {
            ...
            // 检查是否包含id字段
            ConfigFieldInfo idConfigField = fieldInfos.Find((ele) => { return ele.Field.ToLower() == "id"; });
            ...
            scriptDataString.AppendLine($"{SCRIPT_TIP}");
            ...
            scriptTableString.AppendLine($"    private const string ConfigAssetPath = \"{configAssetPath}\";");
            scriptTableString.AppendLine("    public override string AssetPath { get { return ConfigAssetPath; } }");
            ...
            scriptTableString.AppendLine($"    private const string ConfigLoadPath = \"{configLoadPath}\";");
            scriptTableString.AppendLine("    public override string LoadPath { get { return ConfigLoadPath; } }");
            ...
            scriptTableString.AppendLine($"    protected override void Init(string tableContent)");
            scriptTableString.AppendLine("    {");
            scriptTableString.AppendLine($"        m_dataDicById = new Dictionary<{idType}, {dataClassName}>();");
            scriptTableString.AppendLine(
                $"        TableData = JsonConvert.DeserializeObject<List<{dataClassName}>>(tableContent);");
            scriptTableString.AppendLine($"        for (var i = 0; i < TableData.Count; i++)");
            scriptTableString.AppendLine("        {");
            scriptTableString.AppendLine($"            m_dataDicById.Add(TableData[i].id, TableData[i]);");
            scriptTableString.AppendLine("        }");
            scriptTableString.AppendLine("    }");
        ...
        public static void GetAllExcelFiles(string folderPath, List<string> configFiles)
        {
            ...
            {
                GetAllExcelFiles(dd.FullName, configFiles);
            }
        }
